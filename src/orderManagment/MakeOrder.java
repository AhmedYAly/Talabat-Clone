...
public class MakeOrder extends JFrame {
    ...
    private void fillMEALComboBox() {
        ...
        try {
            ...
            select_products.setString(1, (String) restaurant.getSelectedItem());
    ...
    public void totalPriceButtonActionPerformed(ActionEvent evt) throws SQLException {
        ...
        float total = 0;
        select_product.setString(1, (String) mealName.getSelectedItem());
        ResultSet price = select_product.executeQuery();
        while (price.next()) {
            total = price.getFloat("PRODUCT_PRICE");
        }
        JOptionPane.showMessageDialog(new MakeOrder(username), total * Integer.valueOf(quantity.getText()), "total price", JOptionPane.INFORMATION_MESSAGE);
    }
}